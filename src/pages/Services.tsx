      ...
      <main className="flex-1">
        ...
        <section className="relative bg-navy pt-24 pb-16 md:pt-32 md:pb-24">
          ...
          <div className="container-wide relative z-10">
            <motion.div {...motionProps} variants={staggerContainer} className="max-w-3xl mx-auto text-center">
              <motion.h1 variants={fadeUp} className="font-display text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-4">
                Eco-Friendly Mining{" "}
                <span className="text-gold">Solutions</span>
              </motion.h1>
              <motion.p variants={fadeUp} className="font-display text-xl md:text-2xl text-gold/80 tracking-wide mb-6">
                Design · Develop · Deploy
              </motion.p>
              <motion.p variants={fadeUp} className="font-inter text-lg text-white/80 leading-relaxed mb-8">MineTeck analyzes and designs customized flowsheets to safely and responsibly recover precious metals without toxic chemicals or environmental harm. Our zero-waste approach combines proven gravity separation technology with advanced water management systems, delivering regulatory compliance and operational efficiency from day one.</motion.p>
              <motion.div variants={fadeUp}>
                <Link to="/access#services">
                  <Button variant="gold" size="lg" className="font-inter font-semibold hover:shadow-lg hover:shadow-gold/25 hover:-translate-y-0.5 transition-all duration-200">
                    Request Services
        ...
        <section className="py-16 md:py-24 bg-background">
          <div className="container-wide">
            <motion.div {...motionProps} variants={staggerContainer} className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
              {SERVICES.map(service => {
              const IconComponent = service.icon;
              return <motion.div key={service.id} variants={staggerItem}>
                    <Card className="h-full bg-card border-border hover:border-gold/30 transition-colors duration-300 group">
                      ...
                      <CardContent className="pb-4">
                        <ul className="space-y-2">
                          {service.services.map(item => <li key={item} className="flex items-center gap-2 font-inter text-sm text-foreground/80">
                      ...
                      <CardFooter>
                        <Link to="/access#services" className="w-full">
                          <Button variant="outline" className="w-full border-gold/50 text-gold hover:bg-gold hover:text-navy font-inter font-medium transition-all duration-200">
                            Request Services
        ...
        <section className="py-16 md:py-24 bg-navy">
          <div className="container-wide">
            <motion.div {...motionProps} variants={staggerContainer} className="max-w-2xl mx-auto text-center">
              <motion.h2 variants={fadeUp} className="font-display text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-6">
                Ready to Design Your{" "}
                <span className="text-gold">Zero-Waste Operation?</span>
              </motion.h2>
              <motion.p variants={fadeUp} className="font-inter text-lg text-white/80 leading-relaxed mb-8">
